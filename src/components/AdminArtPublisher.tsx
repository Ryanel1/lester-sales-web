...
import { BrandOption, emptySource, localDateTime, PublisherAuth, PublisherHeader, PublisherScheduleField, PublisherSourcePanel, rowSource, scheduleDescription, SourceDraft, StoredResource, usePublisherSession } from "@/components/PublisherShared";
...
type ArtGroupSummary = { id: string; brand_id: string; title: string; status: "draft" | "scheduled" | "published" | "archived"; publish_at: string | null; display_order: number; portal_brands: { name: string } | null; portal_art_resources: StoredResource[] };
...
export function AdminArtPublisher() {
  ...
  function reset() { setEditingId(null); setTitle(""); setPublishAt(""); setResources([{ key: crypto.randomUUID(), label: "", source: emptySource() }]); }
  function edit(group: ArtGroupSummary) { setEditingId(group.id); setBrandId(group.brand_id); setTitle(group.title); setPublishAt(localDateTime(group.publish_at)); setResources([...group.portal_art_resources].sort((a, b) => a.display_order - b.display_order).map((resource) => ({ key: resource.id, id: resource.id, label: resource.label, source: rowSource(resource) }))); setMessage(`Editing ${group.title}.`); document.getElementById("art-editor")?.scrollIntoView({ behavior: "smooth" }); }
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body = { brandId, title, status, publishAt: publishAtIso, resources: resources.map((resource, index) => ({ id: resource.id, label: resource.label, kind: "art", ...prepared[index] })) };
      ...
      setMessage(status === "published" ? "Art group published to the customer site now." : status === "scheduled" ? `Art group scheduled for ${scheduleDescription(publishAtIso)}.` : "Art group draft saved privately."); reset(); await load(session);
  ...
  async function action(group: ArtGroupSummary, actionName: "publish" | "unpublish" | "archive" | "move_up" | "move_down" | "duplicate" | "delete") {
    if (!session) return;
    if (["delete", "archive", "unpublish"].includes(actionName) && !window.confirm(actionName === "delete" ? `Permanently delete “${group.title}”?` : actionName === "unpublish" && group.status === "scheduled" ? `Unschedule “${group.title}”? It will return to drafts.` : `${actionName === "archive" ? "Archive" : "Unpublish"} “${group.title}”?`)) return;
    setBusy(true);
    try {
  ...
  return <main className="publisherShell"><PublisherHeader active="art" client={client} description="Group customer-ready artwork by brand and keep every source current." title="Art library manager" /><div className="publisherGrid"><form className="publisherForm" id="art-editor" onSubmit={save}>
    ...
    <section className="publisherPanel"><div className="publisherPanelHeading"><span>01</span><div><h2>Collection</h2><p>Use a customer-friendly group name that matches how the artwork is sold.</p></div></div><div className="publisherFields publisherFieldsTwo"><label>Brand<select onChange={(event) => setBrandId(event.target.value)} required value={brandId}>{brands.map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label><label>Group title<input onChange={(event) => setTitle(event.target.value)} placeholder="Decoration" required value={title} /></label><PublisherScheduleField onChange={setPublishAt} value={publishAt} /></div></section>
    ...
    <div className="publisherSubmitBar"><p aria-live="polite">{message || "Published groups appear as compact customer links on the brand page."}</p><div><button disabled={busy} name="status" type="submit" value="draft">Save draft</button><button disabled={busy || !publishAt} name="status" type="submit" value="scheduled">{editingId ? "Save schedule" : "Schedule"}</button><button className="publisherPrimary" disabled={busy} name="status" type="submit" value="published">{editingId ? "Save & publish now" : "Publish now"}</button></div></div>
  </form><aside className="publisherLibrary"><div><p className="publisherEyebrow">Art library</p><h2>{groups.length} group{groups.length === 1 ? "" : "s"}</h2></div><div className="publisherCatalogRows">{groups.map((group) => <article key={group.id}><div className="publisherCatalogSummary"><div><strong>{group.title}</strong><span suppressHydrationWarning>{group.portal_brands?.name} · {group.portal_art_resources.length} resources{group.status === "scheduled" ? ` · publishes ${scheduleDescription(group.publish_at)}` : ""}</span></div><em data-status={group.status}>{group.status}</em></div><div className="publisherCatalogActions"><button disabled={busy} onClick={() => edit(group)} type="button">Edit</button><button disabled={busy} onClick={() => action(group, "duplicate")} type="button">Duplicate</button><button aria-label={`Move ${group.title} up`} disabled={busy} onClick={() => action(group, "move_up")} type="button">↑</button><button aria-label={`Move ${group.title} down`} disabled={busy} onClick={() => action(group, "move_down")} type="button">↓</button>{group.status === "published" ? <button disabled={busy} onClick={() => action(group, "unpublish")} type="button">Unpublish</button> : group.status === "scheduled" ? <button disabled={busy} onClick={() => action(group, "unpublish")} type="button">Unschedule</button> : <button disabled={busy} onClick={() => action(group, "publish")} type="button">Publish now</button>}<button disabled={busy} onClick={() => action(group, "archive")} type="button">Archive</button><button className="publisherDanger" disabled={busy} onClick={() => action(group, "delete")} type="button">Delete</button></div></article>)}</div></aside></div></main>;
}
