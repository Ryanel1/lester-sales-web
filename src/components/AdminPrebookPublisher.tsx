...
import { BrandOption, emptySource, localDateTime, PublisherAuth, PublisherHeader, PublisherScheduleField, PublisherSourcePanel, rowSource, scheduleDescription, slugify, SourceDraft, StoredResource, usePublisherSession } from "@/components/PublisherShared";
import { validateFuturePublishAt } from "@/lib/catalog-publisher";
...
export function AdminPrebookPublisher() {
  ...
  function reset() { setEditingId(null); setTitle(""); setSlug(""); setSlugEdited(false); setSeason(""); setDeadline(""); setShipDate(""); setMinimums(""); setDetails(""); setImageAlt(""); setPublishAt(""); setHero(emptySource()); setResources(initialResources()); }

  function edit(prebook: PrebookSummary) {
    setEditingId(prebook.id); setBrandId(prebook.brand_id); setTitle(prebook.title); setSlug(prebook.slug); setSlugEdited(true); setSeason(prebook.season); setDeadline(localDateTime(prebook.deadline)); setShipDate(prebook.ship_date); setMinimums(prebook.minimums); setDetails(prebook.details.join("\n")); setImageAlt(prebook.image_alt); setPublishAt(localDateTime(prebook.publish_at));
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const status = submitter?.value === "published" ? "published" : submitter?.value === "scheduled" ? "scheduled" : "draft";
    let publishAtIso: string | null = null;
    try { if (status === "scheduled") publishAtIso = validateFuturePublishAt(publishAt); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Choose a future publication time."); return; }
    setBusy(true); setMessage(status === "published" ? "Uploading and publishing now…" : status === "scheduled" ? "Uploading and scheduling…" : "Uploading and saving draft…");
    try {
      ...
      const body = { brandId, title, slug, season, deadline: new Date(deadline).toISOString(), shipDate, minimums, details: details.split("\n"), imageAlt, hero: preparedHero, status, publishAt: publishAtIso, resources: resources.map((resource, index) => ({ id: resource.id, label: resource.label, kind: resource.kind, ...preparedResources[index] })) };
      ...
      setMessage(status === "published" ? "Prebook published now and visible until its deadline." : status === "scheduled" ? `Prebook scheduled for ${scheduleDescription(publishAtIso)}.` : "Prebook draft saved privately."); reset(); await load(session);
  ...
  async function action(prebook: PrebookSummary, actionName: "publish" | "unpublish" | "archive" | "move_up" | "move_down" | "duplicate" | "delete") {
    if (!session) return;
    if (["delete", "archive", "unpublish"].includes(actionName) && !window.confirm(actionName === "delete" ? `Permanently delete “${prebook.title}”?` : actionName === "unpublish" && prebook.status === "scheduled" ? `Unschedule “${prebook.title}”? It will return to drafts.` : `${actionName === "archive" ? "Archive" : "Unpublish"} “${prebook.title}”?`)) return;
    setBusy(true);
    try {
  ...
  return <main className="publisherShell"><PublisherHeader active="prebooks" client={client} description="Publish limited-time programs with dates, minimums, and every order file together." title="Prebook builder" /><div className="publisherGrid"><form className="publisherForm" id="prebook-editor" onSubmit={save}>
    ...
    <div className="publisherSubmitBar"><p aria-live="polite">{message || "Prebooks disappear automatically when the booking deadline passes."}</p><div><button disabled={busy} name="status" type="submit" value="draft">Save draft</button><button disabled={busy || !publishAt} name="status" type="submit" value="scheduled">{editingId ? "Save schedule" : "Schedule"}</button><button className="publisherPrimary" disabled={busy} name="status" type="submit" value="published">{editingId ? "Save & publish now" : "Publish now"}</button></div></div>
  </form><aside className="publisherLibrary"><div><p className="publisherEyebrow">Program library</p><h2>{prebooks.length} prebook{prebooks.length === 1 ? "" : "s"}</h2></div><div className="publisherCatalogRows">{prebooks.map((prebook) => <article key={prebook.id}><div className="publisherCatalogSummary"><div><strong>{prebook.title}</strong><span suppressHydrationWarning>{prebook.portal_brands?.name} · closes {new Date(prebook.deadline).toLocaleDateString()}{prebook.status === "scheduled" ? ` · publishes ${scheduleDescription(prebook.publish_at)}` : ""}</span></div><em data-status={prebook.status}>{prebook.status}</em></div><div className="publisherCatalogActions"><button disabled={busy} onClick={() => edit(prebook)} type="button">Edit</button><button disabled={busy} onClick={() => action(prebook, "duplicate")} type="button">Duplicate</button><button aria-label={`Move ${prebook.title} up`} disabled={busy} onClick={() => action(prebook, "move_up")} type="button">↑</button><button aria-label={`Move ${prebook.title} down`} disabled={busy} onClick={() => action(prebook, "move_down")} type="button">↓</button>{prebook.status === "published" ? <button disabled={busy} onClick={() => action(prebook, "unpublish")} type="button">Unpublish</button> : prebook.status === "scheduled" ? <button disabled={busy} onClick={() => action(prebook, "unpublish")} type="button">Unschedule</button> : <button disabled={busy} onClick={() => action(prebook, "publish")} type="button">Publish now</button>}<button disabled={busy} onClick={() => action(prebook, "archive")} type="button">Archive</button><button className="publisherDanger" disabled={busy} onClick={() => action(prebook, "delete")} type="button">Delete</button></div></article>)}</div></aside></div></main>;
}
